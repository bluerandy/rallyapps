(function()
{
    ...
    Ext.define('Rally.data.wsapi.TreeStore', {
            ...
            currentPage : 1,
            /**
             * @cfg {Number} The number of records to retrieve per page.
             */
            pageSize : 25,
            /**
             ...
             * 
             * @cfg {Object} context
             * @cfg {String} context.workspace The ref of the workspace to scope
             *      to
             * @cfg {String} context.project The ref of the project to scope to.
             *      Specify null to query the entire specified workspace.
             * @cfg {Boolean} context.projectScopeUp Whether to scope up the
             *      project hierarchy
             * @cfg {Boolean} context.projectScopeDown Whether to scope down the
             *      project hierarchy
             */
            context : undefined,
            /**
             * The wsapi version to use when automatically retrieving a model
             * before loading. By default the wsapi version of the specified
             * model is used.
             * 
            ...
            nodeParam : undefined,
            /**
             * The model for handling root level and child level items
             * (required)
             * 
            ...
            parentTypes : [],
            /**
             * @cfg {String[]} fetch The fields to be retrieved using
             *      shallowFetch instead of a regular fetch.
             */
            fetch : undefined,
            /**
             * @cfg {Boolean}
             * 
             * True to set record's 'leaf' property to true when children are
             * present.
             */
            ...
            constructor : function(config)
            {
                ...
                this.addEvents(
                /**
                 * @event error Fires when a store load comes back with errors
                 * @param {String[]}
                 *            errors
                 */
                'error',
                /**
                 * @event currentPageReset Fires when the store determines that
                 *        the current page needs to be reset to the first page
                 *        of results
                 */
                'currentPageReset', 'parenttypeschange');
            ...
            _configureTopLevelLoad : function(options)
            {
                ...
                // options.filter =
                // Rally.data.wsapi.Filter.or(_(this.parentTypes).map(function(type)
                // {
                ...
                // return null;
                // }
                // }, this).filter(function (r) { return r !== null;
                // }).value());
            },
            _getChildNodeFilters : function(node)
            {
                // me.logger.log("TreeStore timeFilter: ", this.timeFilter);
             ...
             * {@link Ext.data.Model record} in the store, walking down the
             * entire tree.
             * 
             * @param {Function}
             *            fn The function to call. The
             *            {@link Ext.data.Model Record} is passed as the first
             *            parameter. Returning `false` aborts and exits the
             *            iteration.
             * @param {Object}
             *            [scope] The scope (this reference) in which the
             *            function is executed. Defaults to the current
             *            {@link Ext.data.Model record} in the iteration.
             */
             ...
             *            record Record to match against. Can also be an object
             *            with a _ref property or a ref string or an id.
             */
            ...
            /**
             * Reload the specified record. The current store filters will also
             * be applied.
             * 
             * @param {Rally.data.WsapiModel}
             *            record the record to reload
             * @param {Object}
             *            options additional options to be applied to the
             *            {Ext.data.Operation}.
             * @param {Function}
            ...
            reloadRecord : function(record, options)
            {
                ...
                operationConfig.fetch = this.fetch ? _.union(this.fetch, _.pluck(modelToLoadFrom.getAssociationFields(), 'name')) : this._buildFetch(this.fetch, modelToLoadFrom);
            ...
            /**
             * Determines if a given record matches the store filters and would
             * be placed at the top level of the tree.
             * 
             * @param {Rally.domain.WsapiModel}
             *            record The record to interrogate
             * @return {Deft.promise.Promise} A promise that resolves to the
             *         boolean answer
             */
            doesRecordMatchStoreFilters : function(record)
            {
                var deferred = Ext.create('Deft.promise.Deferred'), operation = new Ext.data.Operation({
                        ...
                        fetch : false,
                        params : {
                            query : Rally.data.wsapi.Filter.and(this.filters.getRange().concat([
            ...
            _buildFetch : function(fetch, model)
            {
                ...
                fetch = _.union(_.isArray(fetch) ? fetch : fetch.split(","), [
                                'ObjectID',
                                'VersionId'
                ]);
