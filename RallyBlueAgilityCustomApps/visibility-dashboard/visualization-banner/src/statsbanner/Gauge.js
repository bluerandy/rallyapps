(function() {
	...
	/**
	 * gauge chart for stats banner abstract class
	 */
	Ext.define('Rally.apps.releasetracking.statsbanner.Gauge', {
		...
		requires : [ 'Rally.ui.chart.Chart', 'Rally.util.Timebox', 'Rally.util.Colors' ],

		config : { context : null, store : null },
		...
		_addEmptyChart : function() {
			this._cleanupChart();
			this
					._addChart({ chartData : { series : [ { data : [ {
						name : '',
						y : 100,
			...
			this.chart = Ext.create('Rally.ui.chart.Chart', Ext.apply({
				...
				chartConfig : {
					...
					plotOptions : { pie : {
						...
						size : height - 4,
						innerSize : height - 14,
						enableMouseTracking : false, // turns
						shadow : false } },
					title : '',
					tooltip : { enabled : false } } }, chartConfig));
		},
			...
			return this._getTZOffset().then(
					{
						success : function(tzOffset) {
							...
							if (timebox) {
								return Rally.util.Timebox.getCounts(timebox.get('ReleaseStartDate'), timebox
										.get('ReleaseDate'),
										this.getContext().getWorkspace().WorkspaceConfiguration.WorkDays, tzOffset);
							} else {
								return { remaining : 0, workdays : 0 };
							}
						},
						scope : this });
		},
				...
				Rally.environment.getIoProvider().httpGet(
						{
							...
							params : { includeSchema : true, pagesize : 1, fetch : false, project : projectRef },
		...
		getAcceptanceData : function() {
			var acceptanceData = { accepted : 0, total : 0, acceptedCount : 0, count : 0 };
		...
		getEstimatedData : function() {
			var acceptanceData = { accepted : 0, total : 0 };
