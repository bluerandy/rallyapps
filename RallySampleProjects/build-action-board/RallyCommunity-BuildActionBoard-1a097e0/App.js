...
Ext.define('CustomApp', {
        ...
        _createQuery : function(buildDefRecords)
        {
            ...
            // value: '/slm/webservice/1.37/builddefinition/6035424766'
            // //PacSystems Mainline Build Definition
            });
            ...
            Ext.create('Rally.data.WsapiDataStore', {
                    ...
                    listeners : {
                            load : function(store, records, success)
                            {
                                ...
                                {
                                    // console.log("Name: %s, Def: %s, Status
                                    // %s", item.get("Number"),
                                    // item.get("BuildDefinition")._ref,
                                    // item.get("Status"));
                                });
            ...
            Ext.create('Rally.data.WsapiDataStore', {
                    ...
                    listeners : {
                            load : function(store, records, success)
                            {
                                ...
                                // console.log("Build definitions array ",
                                // this.buildDefinitions);
        ...
        _siftBuilds : function(buildDefBuildRecords)
        {
                ...
                Ext.Array.forEach(buildStructure[item].builds, function(item)
                {
                    ...
                    if (build.get("Status") === "FAILURE")
                    {
                        if (buildStructure[buildDef].lastGoodBuild == undefined)
                        {
                            ...
                            // console.log("%s failed, failCount
                            // %d",build.get("Number"),
                            // buildStructure[buildDef].failCount);
                        } else
                        {
                            return;
                        }
                    ...
                    {
                        // console.log("%s succeeded, failCount
                        // %d",build.get("Number"),
                        // buildStructure[buildDef].failCount);
            ...
            Ext.Array.forEach(this.buildDefinitions, function(item)
            {
                ...
                gridFormattedBuilds[count].Name = buildStructure[buildDef].buildDefName; // i.e
                                                                                            // :
                                                                                            // "PACSystems
                                                                                            // Mainline
                                                                                            // CI
            ...
            // Color
            var statusTpl =
                            new Ext.XTemplate("<tpl switch='Status'>", "<tpl case='SUCCESS'>", "<div style='background-color:#07C600; width:100%; padding: 3px'> {Status}; </div>", "<tpl default>",
                                            "<div style='background-color:#FF0000; width:100%; padding: 3px'> {Status}; </div>", "</tpl>");
            var currentBuildTpl =
                            new Ext.XTemplate("<tpl switch='Status'>", "<tpl case='SUCCESS'>",
                                            "<div style='background-color:#07C600; color:#FFF;font-weight:bold; width:100%; padding: 3px'> {CurrentBuild} </div>", "<tpl default>",
                                            "<div style='background-color:#FF0000; color:#FFF;font-weight:bold; width:100%; padding: 3px'> {CurrentBuild} </div>", "</tpl>");
            var lastGoodBuildTpl = new Ext.XTemplate("<tpl>", "<div style='background-color:#07C600; color:#FFF;font-weight:bold; width:100%; padding: 3px'> {LastGoodBuild} </div>", "</tpl>");
            var myGrid = Ext.create('Ext.grid.Panel', {
                    ...
                    columns : [
                                    ...
                                    // flex: 1, xtype: 'templatecolumn', tpl:
                                    // statusTpl},
                                    {
                                            text : 'Current Build',
                                    ...
                                    {
                                            text : 'Successes',
                                            dataIndex : 'NumSucceededBuilds',
                                            width : 50,
                                            tooltip : 'Number of failed builds since the last success',
                                            tooltipType : "qtip"
                                    },
                    // { menuDisabled: true, width: 80, sortable: false, xtype:
                    // 'actioncolumn',items: [{
                    // icon:
                    // 'https://raw.github.com/skandl/BuildActionBoard/master/button_build_text.jpg',
                    // iconCls: 'gotItButton',
                    // tooltip: 'I got it',
                    ...
                    // 'actioncolumn',items: [{
                    // icon:
                    // 'https://raw.github.com/skandl/BuildActionBoard/master/button_got_it_text.jpg',
                    // iconCls: 'gotItButton',
                    // tooltip: 'I got it',
                    // handler: function(grid, rowIndex, colIndex) {
                    // console.log("Selected row %d", rowIndex);
                    ...
                    listeners : {
                        cellclick : function(table, td, cellIndex, record, tr, rowIndex)
                        {
                            ...
                            if (cellIndex === 1)
                            {
                                ...
                                Rally.environment.getMessageBus().publish('buildDefinitionSelected', gridFormattedBuilds[rowIndex].BuildDefinitionRef);
                                // Publish the number of days since the last
                                // working build
...
/*
 * // Creates a table using Rally ui Rally.data.ModelFactory.getModel({
 * type:'Build Definition', scope:this, success: function(model){
 * 
 * var myGrid = Ext.create('Rally.ui.grid.Grid',{ model: model, columnCfgs: [
 * 'Name', 'Description', 'LastStatus' ] }); this.add(myGrid);
 * 
 *  }, context: { workspace: "/workspace/4365660833" }
 * 
 * });
 */
});
