(function() {
	...
	/**
	 * A data store which can retrieve hierarchical artifact data. In general
	 * this class will not be instantiated directly but will instead be created
	 * by Rally.data.wsapi.TreeStoreBuilder:
	 * 
	 * Ext.create('Rally.data.wsapi.TreeStoreBuilder').build({ models:
	 * ['userstory'], autoLoad: true, enableHierarchy: true }).then({ success:
	 * function(store) { //use the store } });
	 * 
	 * @experimental
	 */
	Ext
			.define(
					'Rally.data.wsapi.TreeStore',
					{
						...
						requires : [ 'Deft.promise.Deferred', 'Rally.data.ModelFactory' ],
						...
						statics : {
							...
							childToParentTypeMap : {
								...
								hierarchicalrequirement : [ 'Parent', 'PortfolioItem' ], // NOTE:
								// needs
								// to be added
								// for generic
								// case, but
							...
							parentChildTypeMap : {
								hierarchicalrequirement : [
										...
										{ typePath : 'hierarchicalrequirement', collectionName : 'Children' }, // NOTE:
										// will
										// not
										// work
										// until
										// childToParentTypeMap has
										// hierarchicalrequirement added in
							...
							getChildModelTypePaths : function(parentTypes) {
								...
								return _.reduce(Ext.Array.from(parentTypes),
										function(childTypes, parentType) {
											// console.log(childTypes,
											// parentType,
											// this.parentChildTypeMap[parentType]);
											return _.union(childTypes, _.pluck(this.parentChildTypeMap[parentType],
													"typePath"));
										}, [], this);
							} },

						/**
						 * @cfg {Number} The starting page to be retrieved
						 */
						currentPage : 1,

						/**
						 * @cfg {Number} The number of records to retrieve per
						 *      page.
						 */
						pageSize : 25,
						 ...
						 * 
						 * @cfg {Object} context
						 * @cfg {String} context.workspace The ref of the
						 *      workspace to scope to
						 * @cfg {String} context.project The ref of the project
						 *      to scope to. Specify null to query the entire
						 *      specified workspace.
						 * @cfg {Boolean} context.projectScopeUp Whether to
						 *      scope up the project hierarchy
						 * @cfg {Boolean} context.projectScopeDown Whether to
						 *      scope down the project hierarchy
						 */
						context : undefined,

						/**
						 * The wsapi version to use when automatically
						 * retrieving a model before loading. By default the
						 * wsapi version of the specified model is used.
						 * 
						...
						/**
						 * The model for handling root level and child level
						 * items (required)
						 * 
						...
						/**
						 * The attribute types to render as root items
						 * (required)
						 * 
						...
						/**
						 * @cfg {String[]} fetch The fields to be retrieved
						 *      using shallowFetch instead of a regular fetch.
						 */
						fetch : undefined,
						 ...
						 * @cfg {Boolean}
						 * 
						 * True to set record's 'leaf' property to true when
						 ...
						 * @cfg {Boolean}
						 * 
						 * False to have expanding nodes ignore Project Scope
						 * settings.
						 */
						...
						constructor : function(config) {
							...
							if (this.enableHierarchy) {
								this.childLevelSorters = [
										...
										{ property : Rally.data.Ranker.getRankField(this.model), direction : 'ASC' } ];
							}
							this.fetch = this._buildFetch(this.fetch, this.model);

							this.addEvents(
							/**
							 * @event error Fires when a store load comes back
							 *        with errors
							 * @param {String[]}
							 *            errors
							 */
							...
							/**
							 * @event currentPageReset Fires when the store
							 *        determines that the current page needs to
							 *        be reset to the first page of results
							 */
							'currentPageReset',
						...
						getAllParentFieldTypes : function() {
							// going from this --> { defect: ['a', 'b'],
							// userstory: ['c'] }
							// to this --> ['a', 'b', 'c']
							// the apply() is necessary, because values() will
							// return an
							// array of
							// arrays,
							...
							if (this.enableHierarchy) {
								...
								_.each(this.getChildTypes(), function(type) {
									parentFieldNames = _.union(parentFieldNames, this
											.getParentFieldTypesByChildType(type.toLowerCase()));
								}, this);
							}
							...
							if (entp.indexOf('portfolioitem') >= 0 && _.contains(this.getChildTypes(),
									'hierarchicalrequirement')) {
								// console.log('PI -> US');
						...
						getChildTypes : function() {
							if (this.enableHierarchy) {
								...
								return this.self
										.getChildModelTypePaths(Ext.Array.from(this.getExpandingNodeTypePath()));
							}
							return [];
						},
						...
						_getCollectionFetchNames : function() {
							...
							if (this.isRootNode(this.expandingNode)) {
								_.each(Ext.Array.from(this.parentTypes), function(type) {
									collectionFetchNames = _.union(collectionFetchNames,
											this.self.expandedCollectionNames[type.toLowerCase()]);
								}, this);
							} else {
								_.each(Ext.Array.from(this.getChildTypes()), function(type) {
									collectionFetchNames = _.union(collectionFetchNames, _.pluck(
											this.self.parentChildTypeMap[type], "collectionName"));
								}, this);
							}
						...
						_decorateModels : function() {
							if (_.isFunction(this.model.getArtifactComponentModels)) {
								_.each(this.model.getArtifactComponentModels(), Ext.data.NodeInterface.decorate,
						...
						load : function(options) {
							...
							var deferred = Ext.create('Deft.Deferred'), originalCallback = options.callback, me = this;
						...
						_configureLoad : function(options) {
							...
							if (this.expandingNodesRespectProjectScoping || this.isRootNode(this.expandingNode) || (_
									.contains(this.parentTypes, this.getExpandingNodeTypePath()) && !this.expandingNode)) {
						...
						_configureTopLevelLoad : function(options) {
							...
							options.sorters = [ { property : 'PercentDoneByStoryPlanEstimate', direction : 'DESC' } ];
						},

						_getChildNodeFilters : function(node) {
							var childItemTypes = this.self.parentChildTypeMap[node.get('_type')];
							var childTypeMap = Ext.Array.map(childItemTypes, function(childType) {
								return childType.typePath;
							}).join(",");
							...
							filters.push(Rally.data.wsapi.Filter.or(_.map(_.union(this.getAllParentFieldTypes(),
									customTraversal), function(name) {
								return { property : name, operator : '=', value : node.get('_ref') };
							})));
							return filters;
						},
						...
						onProxyLoad : function(operation) {
							...
							// For some reason, when expanding top level
							// stories, the bottom
							// level
							// PI becomes the child.
						 ...
						 * {@link Ext.data.Model record} in the store, walking
						 * down the entire tree.
						 * 
						 * @param {Function}
						 *            fn The function to call. The
						 *            {@link Ext.data.Model Record} is passed as
						 *            the first parameter. Returning `false`
						 *            aborts and exits the iteration.
						 * @param {Object}
						 *            [scope] The scope (this reference) in
						 *            which the function is executed. Defaults
						 *            to the current
						 *            {@link Ext.data.Model record} in the
						 *            iteration.
						 */
						 ...
						 * 
						 * @param {Number/String}
						 *            id the record id or internal id
						 * @return {Ext.data.NodeInterface}
						 */
						getNodeById : function(id) {
							return this.callParent(arguments) || this.findExactRecord(id);
						},

						/**
						 * Finds a record in the store that matches the given
						 * record.
						 * 
						 * @param {Rally.domain.WsapiModel/Object/String/Number}
						 *            record Record to match against. Can also
						 *            be an object with a _ref property or a ref
						 *            string or an id.
						 */
						...
						/**
						 * Reload the specified record. The current store
						 * filters will also be applied.
						 * 
						 * @param {Rally.data.WsapiModel}
						 *            record the record to reload
						 * @param {Object}
						 *            options additional options to be applied
						 *            to the {Ext.data.Operation}.
						 * @param {Function}
						 *            options.success callback -
						 * @deprecated - use returned promise instead
						 * @param {Function}
						 *            options.failure callback -
						 * @deprecated - use returned promise instead
						 * @param {Object}
						 *            options.scope callback scope -
						 * @deprecated - use returned promise instead
						 * @return {Deft.Promise(Rally.data.WsapiModel)}
						 */
						reloadRecord : function(record, options) {
							options = options || {};
							var deferred = Ext.create('Deft.promise.Deferred'), operationConfig = Ext.merge({
								action : 'read',
								limit : 1,
								requester : this,
								context : this.context,
								filters : [ { property : 'ObjectID', value : record.getId() } ],
								params : {} }, options), modelToLoadFrom, recordTypePath;
							...
							operationConfig.fetch = this.fetch ? _.union(this.fetch, _.pluck(modelToLoadFrom
									.getAssociationFields(), 'name')) : this._buildFetch(this.fetch, modelToLoadFrom);

							if (record.parentNode === this.getRootNode()) {
								operationConfig.filters = operationConfig.filters.concat(this.filters.getRange());
							}

							modelToLoadFrom.getProxy().read(Ext.create('Ext.data.Operation', operationConfig),
									function(op) {
						...
						/**
						 * Determines if a given record matches the store
						 * filters and would be placed at the top level of the
						 * tree.
						 * 
						 * @param {Rally.domain.WsapiModel}
						 *            record The record to interrogate
						 * @return {Deft.promise.Promise} A promise that
						 *         resolves to the boolean answer
						 */
						doesRecordMatchStoreFilters : function(record) {
							var deferred = Ext.create('Deft.promise.Deferred'), operation = new Ext.data.Operation({
								action : 'read',
								fetch : false,
								params : { query : Rally.data.wsapi.Filter.and(
										this.filters.getRange().concat(
												[ new Rally.data.wsapi.Filter({
													property : 'ObjectID',
													operator : '=',
													value : Rally.util.Ref.getOidFromRef(record) }) ])).toString() },
								limit : 1,
								requester : this }), callback = function(operation) {
						...
						doesRecordMatchChildTypes : function(record) {
							return _(this.getChildTypes()).invoke('toLowerCase').contains(
									record.self.typePath.toLowerCase());
						},

						_configureProxy : function(forTopLevel) {
							this.proxy = this.model.getProxy();

							var types = _.isFunction(this.model.getArtifactComponentModels) ? this
									._getTypesForLevel(forTopLevel) : this.model.elementName;
						...
						_instrumentRecord : function(record) {
							var leafCount = this._getLeafCount(record), isLeaf = leafCount < 1 || !this.enableHierarchy;
						...
						_getLeafCount : function(record) {
							var typePath = record.get('_type').toLowerCase(), expandedCollectionNames = Ext.Array
									.from(this.self.expandedCollectionNames[typePath]);
