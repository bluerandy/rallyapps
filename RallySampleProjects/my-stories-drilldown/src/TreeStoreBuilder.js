(function()
{
    ...
    { extend : 'Rally.data.DataStoreBuilder', requires :
    [ 'Rally.data.wsapi.TreeStore', 'Rally.data.ModelFactory' ],
    ...
    loadModels : function(config)
    {
        ...
        { success : function(models)
        {
            ...
            _.each(models, function(model)
            {
                ...
                {
                    Rally.data.wsapi.TreeStore.expandedCollectionNames[typePath] =
                    [ 'Children' ];
                    Rally.data.wsapi.TreeStore.childToParentTypeMap[typePath] =
                    [ 'Parent' ];
                    ...
                    if (model.isLowestLevelPortfolioItem())
                    {
                        ...
                        [
                        { typePath : 'hierarchicalrequirement', collectionName : 'UserStories' } ];
                    }
                }
            });
            ...
            _.each(pis, function(model)
            {
                ...
                if (pm)
                {
                    ...
                    [
                    { typePath : typePath, collectionName : 'Children' } ];
                }
            });
    ...
    _loadChildModels : function(modelsToLoad, loadedModels, config)
    {
        ...
        { context : config.context, types : modelsToLoad, requester : config.requester || this }).then(
