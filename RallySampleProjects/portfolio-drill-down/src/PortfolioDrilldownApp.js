(function()
{
    ...
    Ext
            .define(
                    'Rally.apps.portfoliodrilldown.PortfolioDrilldownApp',
                    {
                        ...
                        mixins :
                        [ 'Rally.app.CardFieldSelectable', 'Rally.clientmetrics.ClientMetricsRecordable' ],
                        ...
                        eModelNames :
                        [ 'User Story', 'Defect', 'Defect Suite', 'Test Set' ],
                        sModelNames :
                        [],
                        ...
                        launch : function()
                        {
                            if (!this.rendered)
                            {
                                this.on('afterrender', this.launch, this,
                                { single : true });
                                return;
                            }

                            var typeStore = Ext.create('Rally.data.wsapi.Store',
                            { autoLoad : false, model : 'TypeDefinition', sorters :
                            [
                            { property : 'Ordinal', direction : 'ASC' } ], filters :
                            [
                            { property : 'Parent.Name', operator : '=', value : 'Portfolio Item' },
                        ...
                        {
                            var plugins =
                            [ 'rallygridboardaddnew' ], context = this.getContext();

                            plugins.push('rallygridboardtoggleable');
                            var alwaysSelectedValues =
                            [ 'FormattedID', 'Name', 'Owner', 'plannedStartDate', 'plannedEndDate' ];
                            ...
                            plugins.push(
                            {
                                ...
                                modelNames : this._getModelNames(),
                                boardFieldDefaults : (this.getSetting('cardFields') && this.getSetting('cardFields')
                                        .split(',')) ||
                                [ 'Parent', 'plannedStartDate', 'plannedEndDate', 'PlanEstimate', 'Iteration' ] });
                        ...
                        {
                            var context = this.getContext(), stateString = 'portfolio-tracking-treegrid', stateId = context
