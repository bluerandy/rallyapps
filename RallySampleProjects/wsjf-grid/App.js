...
Ext.define('CustomApp', {
        ...
        componentCls : 'app',
        // items:{ html:'<a href="https://help.rallydev.com/apps/2.0/doc/">App
        // SDK 2.0 Docs</a>'},
        launch : function()
        {
        ...
        {
            // if we don't yet have a PI combo box or if this is anything other
            // than
            ...
            if (this._piCombobox)
            {
                ...
                {
                    var grid = this.down('rallygrid'), store = grid.getStore(), filter = this._getReleaseFilter();
        ...
        _onPICombobox : function()
        {
            if (this._piCombobox)
            {
                ...
                { // disable the ReleaseComboBox if Feature not selected
                ...
                Rally.data.ModelFactory.getModel({
                        ...
                        success : function(model)
                        {
                            ...
                            { // grid exists, reset the model to the correct
                                // PI type
                                this._myGrid.reconfigureWithModel(model);
                                // clear and re-apply filter since reconfiguring
                                // model
                                // doesn't do this
                                ...
                                {
                                    that._calculateScore(records);
                                    that._updateGrid(store);
                                });
                            }
                        },
                        scope : this
        ...
        _calculateScore : function(records)
        {
            ...
            Ext.Array.each(records, function(feature)
            {
                ...
                if (jobSize > 0)
                { // jobSize is the denominator so make sure it's not 0
                    var score;
                    if (!isChecked)
                    {
                    ...
                    { // only update if score changed
                        feature.set('WSJFScore', score); // set score value
                        // in db
                    }
                }
            });
