(function()
{
    ...
    Ext.define("Rally.apps.charts.burndown.BurnDownCalculator", {
            ...
            {
                var completedStates = this.config.completedScheduleStateNames, aggregationType = this.config.chartAggregationType;
                return [
                                {
                                        "as" : "RemainingPoints",
                                        "f" : function(snapshot)
                ...
                return [
                                {
                                        ...
                                        {
                                            var max = summaryMetrics.Scope_max, increments = seriesData.length - 1, incrementAmount;
                                            if (increments === 0)
                                            {
                                                return max;
                                            }
            ...
            runCalculation : function(snapshots)
            {
                ...
                {
                    // if the slope is positive, try using least squares. If
                    // that's also positive, then use the first result
                    var todoData = chartData.series[0].data;
                    var firstTodoIndex = this._firstNonZero(todoData), lastTodoIndex = (todoData.length - 1) - chartData.projections.pointsAddedCount;
                    var results = this._leastSquares(todoData, firstTodoIndex, lastTodoIndex);
                    // override the prediction line only if least squares says
                    // the slope isn't positive
                    if (results.slope <= 0)
                    {
                        ...
                        chartData.series[3].data[firstTodoIndex] =
                                        ((results.slope * firstTodoIndex) + results.yintercept) + (chartData.series[3].data[lastTodoIndex] - ((results.slope * lastTodoIndex) + results.yintercept));
                        chartData.series[3].connectNulls = true;
                        this.projectionsConfig = undefined;
                    } else
                    {
                        // DE18732, if the slope is up, truncate it at 1.25 of
                        // the max Ideal
            ...
            _recomputeIdeal : function(chartData, endDate)
            {
                ...
                if (endDate > lastDate)
                {
                    // the scopeEndDate date wasn't found in the current
                    // categories...we need to extend categories to include it
                    // (honoring "workDays").
                ...
                {
                    // it is in "scope"...set index to the index of the last
                    // workday in scope
                    index = this._indexOfDate(chartData, endDate);
                    if (index === -1)
                    {
                        // it's in "scope", but falls on a non-workday...back up
                ...
                // set first and last point, and let connectNulls fill in the
                // rest
                var i;
