...
Ext.define('CustomApp', {
        ...
        reload : function()
        {
            ...
            async.map([
                app.dropDown.getRecord()
            ], app.loadIterations, function(err, results)
            {
                ...
                async.map([
                    taskConfig
                ], app.wsapiQuery, function(err, results)
                {
                    var tasks = results[0];
                    if (tasks.length === 0)
                    {
                    ...
                    {
                        return (!_.isUndefined(ui.user) && !_.isUndefined(ui.iteration) && (!_.isNull(ui.user) && !_.isNull(ui.iteration)));
                    });
                    ...
                    {
                        app.tasks = tasks;
                        // app.capacities = _.map(results,function(r) { return
                        // r[0];});
        ...
        noData : function()
        {
            app.tree = Ext.create('Ext.container.Container', {
                html : 'No data found'
            });
            app.add(app.tree);
        },
        addTreeGrid : function()
        {
            ...
            _.each(_.keys(g), function(okey)
            {
                ...
                _.each(_.keys(gProjects), function(pKey)
                {
                    ...
                    _.each(_.keys(gIterations), function(iKey)
                    {
                        ...
                        {
                            return ac.get("User")._refObjectName == okey && ac.get("Project")._refObjectName == pKey && ac.get("Iteration")._refObjectName == iKey;
                        });
            ...
            {
                // console.log("x",x,this.level,this.parent
                // ?this.parent.level:"");
